...
class Ghost:
    def __init__(self, x_coord, y_coord, target, speed, img, direct, dead, box, id):
        ...
        self.rect = self.draw()
        self.move_method = self.select_move_method()

    def select_move_method(self):
        if self.id == 0:  # Blinky - BFS
            return self.move_blinky
        elif self.id == 1:  # Inky
            return self.move_inky
        elif self.id == 2:  # Pinky (A*)
            return self.move_pinky
        elif self.id == 3:  # Clyde
            return self.move_clyde
    ...
    def move_blinky(self):
        ...
        # BFS Pathfinding
        start = (current_x_grid, current_y_grid)
        end = (target_x_grid, target_y_grid)
        ...
        while queue:
            ...
            for dx, dy in [(1,0), (-1,0), (0,1), (0,-1)]:
                ...
                if 0 <= neighbor[0] < len(level[0]) and 0 <= neighbor[1] < len(level):
                    ...
                    if (cell_value < 3 or (cell_value == 9 and (self.in_box or self.dead))) and neighbor not in visited:
        ...
        if self.turns[desired_direction]:
            self.direction = desired_direction
        else:
            # Fallback to original movement if path blocked
            for i in range(4):
                if self.turns[i]:
                    self.direction = i
                    break
    ...
    def move_pinky(self):
        ...
        while open_set:
            ...
            for dx, dy in [(1, 0), (-1, 0), (0, 1), (0, -1)]:
                ...
                if 0 <= neighbor[0] < len(level[0]) and 0 <= neighbor[1] < len(level):
                    cell_value = level[neighbor[1]][neighbor[0]]
                    if cell_value < 3 or (cell_value == 9 and (self.in_box or self.dead)):
...
while run:
    ...
    if moving:
        ...
        if not blinky_dead and not blinky.in_box:
            blinky_x, blinky_y, blinky_direction = blinky.move_method()
        else:
            blinky_x, blinky_y, blinky_direction = blinky.move_clyde()
        if not pinky_dead and not pinky.in_box:
            pinky_x, pinky_y, pinky_direction = pinky.move_method() # move_pinky()
        else:
